...
import { deletePayrollIncome } from "@/redux/pm/payroll-income-slice";
import { useRouter } from "next/navigation";
import { IPayrollIncome } from "@/interfaces/pm";


interface ITablePayrollIncomeProps{
    payrollIncomes: IPayrollIncome[];
}

function TabPayrollIncome({payrollIncomes}: ITablePayrollIncomeProps){
    const dispatch = useAppDispatch();
    const router = useRouter();
    
    const handleDeletePayrollConcept = (id: number) => {
        confirm("¿Estas seguro de eliminar este departamento?") && dispatch(deletePayrollIncome(id));
    }
    return(
        <section className="w-full py-4">
            <div className="grid grid-cols-4 gap-4">
                { payrollIncomes &&   payrollIncomes.map((item: IPayrollIncome, index) => (
                    <div key={index} className={`w-full  px-4 py-3  bg-white  rounded-md shadow-md`}>
                        ...
                        <div>
                            <h1 className="mt-2 text-l font-semibold text-gray-800">{item.date}</h1>
                            <p className="mt-2 text-xs text-gray-600">{item.amount}</p>
                        </div>
                    </div>
                ))}
            </div>
...
export default TabPayrollIncome;